...
include_detailed_annotation_info_helper<-function(data,
  ...
  if ("cluster" %in% annotation_resolution){
    ...
    if("cluster" %in% group_markers_by){
      ...
      dt_top<- unique(dt_top_marker[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","seurat_clusters")][,c("CL_celltype","marker","seurat_clusters","quantile_score_marker","EC_score","specificity","marker_type","annotation_per_cluster")])
      dt_top_marker_by_cl<-dt_top[order(seurat_clusters,-quantile_score_marker)][,head(.SD, n_top_markers),by=c("seurat_clusters")]
      dt_top_marker_by_cl<-as.data.table(dt_top_marker_by_cl)[,c("seurat_clusters","annotation_per_cluster","marker","marker_type","EC_score","specificity","quantile_score_marker")]
      colnames(dt_top_marker_by_cl)<-c("seurat_clusters",eval(name), "marker","marker_type","EC_score","specificity","gene_impact_score_per_cluster")
    ...
    if("celltype_cluster" %in% group_markers_by){
      # for each cluster retrieves first N markers
      dt_top<- unique(dt_top_marker[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","annotation_per_cluster")][,c("CL_celltype","marker","seurat_clusters","quantile_score_marker","EC_score","specificity","marker_type","annotation_per_cluster")])
      dt_top_marker_by_cl<-dt_top[order(seurat_clusters,-quantile_score_marker)][,head(.SD, n_top_markers),by=c("annotation_per_cluster")]
      dt_top_marker_by_cl<-as.data.table(dt_top_marker_by_cl)[,c("seurat_clusters","annotation_per_cluster","marker","marker_type","EC_score","specificity","quantile_score_marker")]
      colnames(dt_top_marker_by_cl)<-c("seurat_clusters",eval(name), "marker","marker_type","EC_score","specificity","gene_impact_score_per_celltype_cluster")
  ...
  if ("cell" %in% annotation_resolution){
    ...
    if("cell" %in% group_markers_by){
      dt_top <- unique(dt_top_marker[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","cell")][,c("CL_celltype","marker","marker_type","quantile_score_marker","cell","score","EC_score","specificity","annotation_per_cell", "diff_score")])
    ...
    if("celltype_cell" %in% group_markers_by){
      ...
      if(!is.null(condition_group_info)){
        if(is.null(CL_celltype_group_info)){ # only condition group
          ...
          colnames(dt_top_marker_by_cell)<-c(eval(name), eval(condition_group_info),eval(CL_celltype_group_info),"marker","marker_type","gene_impact_score_per_celltype_cell","EC_score","specificity")
    ...
    if ("score_cell" %in% group_markers_by){
      ...
      if(!is.null(condition_group_info)){
        if(is.null(CL_celltype_group_info)){ # only condition group
          ...
          dt_top_marker_condition<-merge(dt_top_marker, condition_table, by="cell")
          dt_top <- unique(dt_top_marker_condition[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","annotation_per_cell","condition","CL_celltype")][,c("condition","CL_celltype","annotation_per_cell","marker","marker_type","quantile_score_marker","EC_score","specificity")])
          dt_top<-unique(dt_top[,c("annotation_per_cell","condition","CL_celltype","marker","marker_type","quantile_score_marker","EC_score","specificity")])
          dt_top_marker_by_cell<-dt_top[order(-quantile_score_marker)][,head(.SD, n_top_markers),c("annotation_per_cell","condition")]
        ...
        if(!is.null(CL_celltype_group_info)){
          ...
          dt_top <- unique(dt_top_marker_condition[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","annotation_per_cell","CL_celltype")][,c("CL_celltype","annotation_per_cell","marker","marker_type","quantile_score_marker","EC_score","specificity")])
          dt_top<-unique(dt_top[,c("annotation_per_cell","CL_celltype","marker","marker_type","quantile_score_marker","EC_score","specificity")])
