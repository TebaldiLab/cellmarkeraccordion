...
include_detailed_annotation_info_helper<-function(data,
  ...
  if("NCIT_celltype" %in% colnames(dt_score)){
    func<-"disease"
    names(dt_score)[names(dt_score) == 'NCIT_celltype'] <- 'CL_celltype'
    if(annotation_resolution %in% "cell"){
      names(anno_dt_cell)[names(anno_dt_cell) == 'NCIT_celltype'] <- 'CL_celltype'
      names(final_dt)[names(final_dt) == 'NCIT_celltype'] <- 'CL_celltype'
    } else if(annotation_resolution %in% "cluster"){
      names(final_dt_cluster)[names(final_dt_cluster) == 'NCIT_celltype'] <- 'CL_celltype'
      names(anno_dt_cl)[names(anno_dt_cl) == 'NCIT_celltype'] <- 'CL_celltype'
    }
  ...
  if ("cluster" %in% annotation_resolution){
    ...
    dt_top_marker<-unique(merge.data.table(dt_score,anno_dt_cell_ptc, by=c("CL_celltype","cell")))
    ...
    if("cluster" %in% group_markers_by){
      ...
      dt_top<- unique(dt_top_marker[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","seurat_clusters")][,c("CL_celltype","marker","seurat_clusters","quantile_score_marker","ECs","SPs","marker_type","annotation_per_cluster")])
      dt_top_marker_by_cl<-dt_top[order(seurat_clusters,-quantile_score_marker)][,head(.SD, n_top_markers),by=c("seurat_clusters")]
      dt_top_marker_by_cl<-as.data.table(dt_top_marker_by_cl)[,c("seurat_clusters","annotation_per_cluster","marker","marker_type","ECs","SPs","quantile_score_marker")]
      colnames(dt_top_marker_by_cl)<-c("seurat_clusters",eval(name), "marker","marker_type","ECs","SPs","gene_impact_score_per_cluster")
    ...
    if("celltype_cluster" %in% group_markers_by){
      # for each cluster retrieves first N markers
      dt_top<- unique(dt_top_marker[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","annotation_per_cluster")][,c("CL_celltype","marker","seurat_clusters","quantile_score_marker","ECs","SPs","marker_type","annotation_per_cluster")])
      dt_top_marker_by_cl<-dt_top[order(seurat_clusters,-quantile_score_marker)][,head(.SD, n_top_markers),by=c("annotation_per_cluster")]
      dt_top_marker_by_cl<-as.data.table(dt_top_marker_by_cl)[,c("seurat_clusters","annotation_per_cluster","marker","marker_type","ECs","SPs","quantile_score_marker")]
      colnames(dt_top_marker_by_cl)<-c("seurat_clusters",eval(name), "marker","marker_type","ECs","SPs","gene_impact_score_per_celltype_cluster")
    ...
    if(func == "disease"){
      names(cluster_res_detailed_annotation_info)[names(cluster_res_detailed_annotation_info) == 'CL_celltype'] <- 'NCIT_celltype'
  ...
  if ("cell" %in% annotation_resolution){
    ...
    dt_top_ct_per_cell<-as.data.table(dt_top_ct_by_cell)[,c("cell","CL_celltype","diff_score")]
    ...
    dt_top_marker<-unique(merge.data.table(dt_score,anno_dt_cell, by=c("CL_celltype","cell")))
    dt_top_marker<-dt_top_marker[CL_celltype == annotation_per_cell]

    if("cell" %in% group_markers_by){
      dt_top <- unique(dt_top_marker[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","cell")][,c("CL_celltype","marker","marker_type","quantile_score_marker","cell","score","ECs","SPs","annotation_per_cell", "diff_score")])
      ...
      dt_top_marker_per_cell<-as.data.table(dt_top_marker_by_cell)[,c("cell","annotation_per_cell","diff_score","marker","marker_type","ECs","SPs","score")]
      colnames(dt_top_marker_per_cell)<-c("cell",eval(name),eval(name_score), "marker","marker_type","ECs","SPs", "gene_impact_score_per_cell")
    ...
    if("celltype_cell" %in% group_markers_by){
      ...
      if(!is.null(condition_group_info)){
        if(is.null(celltype_group_info)){ # only condition group
          ...
          dt_top <- unique(dt_top_marker_condition[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","annotation_per_cell","condition")][,c("condition","annotation_per_cell","marker","marker_type","quantile_score_marker","ECs","SPs")])
          dt_top<-unique(dt_top[,c("annotation_per_cell","condition","marker","marker_type","quantile_score_marker","ECs","SPs")])
          dt_top_marker_by_cell<-dt_top[order(-quantile_score_marker)][,head(.SD, n_top_markers),c("annotation_per_cell","condition")]

          name<-paste0(annotation_name,"_per_cell")
          name_score<-paste0(annotation_name,"_per_cell_score")

          colnames(dt_top_marker_by_cell)<-c(eval(name), eval(condition_group_info),"marker","marker_type","gene_impact_score_per_celltype_cell","ECs","SPs")
          ...
          dt_top_marker_condition<-merge(dt_top_marker, condition_table, by="cell")
          dt_top <- unique(dt_top_marker_condition[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","annotation_per_cell","condition","celltype")][,c("condition","celltype","annotation_per_cell","marker","marker_type","quantile_score_marker","ECs","SPs")])
          dt_top<-unique(dt_top[,c("annotation_per_cell","condition","celltype","marker","marker_type","quantile_score_marker","ECs","SPs")])
          dt_top_marker_by_cell<-dt_top[order(-quantile_score_marker)][,head(.SD, n_top_markers),c("annotation_per_cell","condition","celltype")]

          name<-paste0(annotation_name,"_per_cell")
          name_score<-paste0(annotation_name,"_per_cell_score")

          colnames(dt_top_marker_by_cell)<-c(eval(name), eval(condition_group_info),eval(celltype_group_info),"marker","marker_type","gene_impact_score_per_celltype_cell","ECs","SPs")
        ...
        if(!is.null(celltype_group_info)){
          ...
          dt_top_marker_condition<-merge(dt_top_marker, condition_table, by="cell")
          dt_top <- unique(dt_top_marker_condition[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","annotation_per_cell","celltype")][,c("annotation_per_cell","marker","marker_type","quantile_score_marker","ECs","SPs")])
          dt_top<-unique(dt_top[,c("annotation_per_cell","marker","marker_type","quantile_score_marker","ECs","SPs")])
          dt_top_marker_by_cell<-dt_top[order(-quantile_score_marker)][,head(.SD, n_top_markers),c("annotation_per_cell","condition")]
      ...
      if(is.null(celltype_group_info) & is.null(condition_group_info)){
        dt_top <- unique(dt_top_marker[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","annotation_per_cell")][,c("cell","annotation_per_cell","marker","marker_type","quantile_score_marker","ECs","SPs")])
        dt_top_marker_by_cell<-unique(dt_top[,c("annotation_per_cell","marker","marker_type","quantile_score_marker","ECs","SPs")])
        dt_top_marker_by_cell<-dt_top_marker_by_cell[order(-quantile_score_marker)][,head(.SD, n_top_markers),annotation_per_cell]
        name<-paste0(annotation_name,"_per_cell")
        name_score<-paste0(annotation_name,"_per_cell_score")

        colnames(dt_top_marker_by_cell)<-c(eval(name), "marker","marker_type","gene_impact_score_per_celltype_cell","ECs","SPs")
    ...
    if ("score_cell" %in% group_markers_by){
      ...
      if(!is.null(condition_group_info)){
        if(is.null(celltype_group_info)){ # only condition group
          ...
          colnames(dt_top_marker_by_cell)<-c(eval(name), eval(condition_group_info),"marker","marker_type","gene_impact_score_per_score_cell","ECs","SPs")
          ...
          dt_top_marker_condition<-merge(dt_top_marker, condition_table, by="cell")
          dt_top <- unique(dt_top_marker_condition[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","annotation_per_cell","condition","CL_celltype")][,c("condition","CL_celltype","annotation_per_cell","marker","marker_type","quantile_score_marker","ECs","SPs")])
          dt_top<-unique(dt_top[,c("annotation_per_cell","condition","CL_celltype","marker","marker_type","quantile_score_marker","ECs","SPs")])
          dt_top_marker_by_cell<-dt_top[order(-quantile_score_marker)][,head(.SD, n_top_markers),c("annotation_per_cell","condition")]
        ...
        if(!is.null(celltype_group_info)){
          ...
          dt_top_marker_condition[,quantile_diff_score := quantile((unique(.SD, by = c("cell")))$diff_score, probs = top_cell_score_quantile_threshold, na.rm=TRUE),by="CL_celltype"]
          dt_top_marker_condition<-dt_top_marker_condition[diff_score >= quantile_diff_score]
          dt_top <- unique(dt_top_marker_condition[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","annotation_per_cell","CL_celltype")][,c("CL_celltype","annotation_per_cell","marker","marker_type","quantile_score_marker","ECs","SPs")])
          dt_top<-unique(dt_top[,c("annotation_per_cell","CL_celltype","marker","marker_type","quantile_score_marker","ECs","SPs")])
          dt_top_marker_by_cell<-dt_top[order(-quantile_score_marker)][,head(.SD, n_top_markers),c("annotation_per_cell","CL_celltype")]
    ...
    if(func == "disease"){
      ...
      names(cell_res_detailed_annotation_info)[names(cell_res_detailed_annotation_info) == 'CL_celltype'] <- 'NCIT_celltype'
