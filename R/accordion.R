...
accordion<-function(data,
  ...
  if(is.null(CL_celltypes)){
    ...
    input_CL_celltype<-CL_celltypes
    accordion_marker<-accordion_marker[CL_celltype %in% input_CL_celltype][marker %in% rownames(data)]
    input_CL_celltype_not_in_accordion<-input_CL_celltype[!(input_CL_celltype %in% unique(accordion_marker$CL_celltype))]
  ...
  accordion_marker[,group:=paste0(CL_celltype, "_", marker)]
  ...
  if(!is.null(combined_score_quantile_threshold)){
    if(!is.numeric(combined_score_quantile_threshold) | combined_score_quantile_threshold > 1 | combined_score_quantile_threshold <= 0){
      ...
      accordion_marker[,quantile_combined_score:=quantile(combined_score, probs= combined_score_quantile_threshold), by="CL_celltype"]
  ...
  if ("cluster" %in% annotation_resolution){
    ...
    final_dt_cluster[, quantile_score_cluster:= quantile(diff_score,probs = cluster_score_quantile_threshold, na.rm=TRUE), by=c("seurat_clusters","CL_celltype")]
    if (allow_unknown == T){
      final_dt_cluster[quantile_score_cluster < 0, annotation_per_cell:= "unknown"][quantile_score_cluster > 0, annotation_per_cell := CL_celltype]
    } else {
      final_dt_cluster[, annotation_per_cell := CL_celltype]
    }
    ...
    anno_dt_cl<-anno_dt_cell_ptc[order(-quantile_score_cluster)][,head(.SD, 1),"seurat_clusters"][,-c("cell","CL_celltype","diff_score","ncell_tot_cluster","ncell_celltype_cluster")]
  ...
  if ("cell" %in% annotation_resolution){
    ...
    if (allow_unknown == T){
      anno_dt_cell[diff_score < 0, annotation_per_cell:= "unknown"][diff_score > 0, annotation_per_cell := CL_celltype]

    } else {
      anno_dt_cell[, annotation_per_cell := CL_celltype]
