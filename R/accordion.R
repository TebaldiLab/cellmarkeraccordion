#' Automatically annotating and interpreting single-cell populations with the built-in Cell Marker Accordion database
#'
...
#' objects, is stored in the “misc@accordion” slot. If a count matrix was
...
if(sum(dim(data@assays[[assay]]@counts))!=0){
  ...
  if("cluster" %in% annotation_resolution & plot == T){
    if(data_type == "seurat"){
      data<-accordion_plot(data)
    } else{
      accordion_output<-accordion_plot(accordion_output)
