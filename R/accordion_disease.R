...
accordion_disease<-function(data,
 ...
 # annotation per cluster
  ...
  if("cluster" %in% annotation_resolution & plot == T){
    if(data_type == "seurat"){
      data<-accordion_plot(data, info_to_plot = annotation_name, resolution = resolution)
    } else{
      accordion_output<-accordion_plot(accordion_output, info_to_plot = annotation_name, resolution = resolution)
