...
include_detailed_annotation_info_helper_custom<-function(data,
  ...
  if ("cluster" %in% annotation_resolution){
    ...
    if("cluster" %in% group_markers_by){
      ...
      dt_top<- unique(dt_top_marker[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","seurat_clusters")][,c("cell_type","marker","seurat_clusters","quantile_score_marker","weight","SPs","marker_type","annotation_per_cluster")])
      dt_top_marker_by_cl<-dt_top[order(seurat_clusters,-quantile_score_marker)][,head(.SD, n_top_markers),by=c("seurat_clusters")]
      dt_top_marker_by_cl<-as.data.table(dt_top_marker_by_cl)[,c("seurat_clusters","annotation_per_cluster","marker","marker_type","weight","SPs","quantile_score_marker")]
      colnames(dt_top_marker_by_cl)<-c("seurat_clusters",eval(name), "marker","marker_type","weight","SPs","gene_impact_score_per_cluster")
    ...
    if("celltype_cluster" %in% group_markers_by){
      # for each cluster retrieves first N markers
      dt_top<- unique(dt_top_marker[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","annotation_per_cluster")][,c("cell_type","marker","seurat_clusters","quantile_score_marker","weight","SPs","marker_type","annotation_per_cluster")])
      dt_top_marker_by_cl<-dt_top[order(seurat_clusters,-quantile_score_marker)][,head(.SD, n_top_markers),by=c("annotation_per_cluster")]
      dt_top_marker_by_cl<-as.data.table(dt_top_marker_by_cl)[,c("seurat_clusters","annotation_per_cluster","marker","marker_type","weight","SPs","quantile_score_marker")]
      colnames(dt_top_marker_by_cl)<-c("seurat_clusters",eval(name), "marker","marker_type","weight","SPs","gene_impact_score_per_celltype_cluster")
  ...
  if ("cell" %in% annotation_resolution){
    ...
    if("cell" %in% group_markers_by){
      dt_top <- unique(dt_top_marker[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","cell")][,c("cell_type","marker","marker_type","quantile_score_marker","cell","score","weight","SPs","annotation_per_cell", "diff_score")])
      ...
      dt_top_marker_per_cell<-as.data.table(dt_top_marker_by_cell)[,c("cell","annotation_per_cell","diff_score","marker","marker_type","weight","SPs","score")]
      colnames(dt_top_marker_per_cell)<-c("cell",eval(name),eval(name_score), "marker","marker_type","weight","SPs", "gene_impact_score_per_cell")
    ...
    if("celltype_cell" %in% group_markers_by){
      ...
      if(!is.null(condition_group_info)){
        if(is.null(celltype_group_info)){ # only condition group
          ...
          dt_top <- unique(dt_top_marker_condition[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","annotation_per_cell","condition")][,c("condition","annotation_per_cell","marker","marker_type","quantile_score_marker","weight","SPs")])
          dt_top<-unique(dt_top[,c("annotation_per_cell","condition","marker","marker_type","quantile_score_marker","weight","SPs")])
          dt_top_marker_by_cell<-dt_top[order(-quantile_score_marker)][,head(.SD, n_top_markers),c("annotation_per_cell","condition")]

          name<-paste0(annotation_name,"_per_cell")
          name_score<-paste0(annotation_name,"_per_cell_score")

          colnames(dt_top_marker_by_cell)<-c(eval(name), eval(condition_group_info),"marker","marker_type","gene_impact_score_per_celltype_cell","weight","SPs")
          ...
          dt_top_marker_condition<-merge(dt_top_marker, condition_table, by="cell")
          dt_top <- unique(dt_top_marker_condition[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","annotation_per_cell","condition","celltype")][,c("condition","celltype","annotation_per_cell","marker","marker_type","quantile_score_marker","weight","SPs")])
          dt_top<-unique(dt_top[,c("annotation_per_cell","condition","celltype","marker","marker_type","quantile_score_marker","weight","SPs")])
          dt_top_marker_by_cell<-dt_top[order(-quantile_score_marker)][,head(.SD, n_top_markers),c("annotation_per_cell","condition","celltype")]

          name<-paste0(annotation_name,"_per_cell")
          name_score<-paste0(annotation_name,"_per_cell_score")

          colnames(dt_top_marker_by_cell)<-c(eval(name), eval(condition_group_info),eval(celltype_group_info),"marker","marker_type","gene_impact_score_per_celltype_cell","weight","SPs")
        ...
        if(!is.null(celltype_group_info)){
          ...
          dt_top_marker_condition<-merge(dt_top_marker, condition_table, by="cell")
          dt_top <- unique(dt_top_marker_condition[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","annotation_per_cell","celltype")][,c("annotation_per_cell","marker","marker_type","quantile_score_marker","weight","SPs")])
          dt_top<-unique(dt_top[,c("annotation_per_cell","marker","marker_type","quantile_score_marker","weight","SPs")])
          dt_top_marker_by_cell<-dt_top[order(-quantile_score_marker)][,head(.SD, n_top_markers),c("annotation_per_cell","condition")]
      ...
      if(is.null(celltype_group_info) & is.null(condition_group_info)){
        dt_top <- unique(dt_top_marker[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","annotation_per_cell")][,c("cell","annotation_per_cell","marker","marker_type","quantile_score_marker","weight","SPs")])
        dt_top_marker_by_cell<-unique(dt_top[,c("annotation_per_cell","marker","marker_type","quantile_score_marker","weight","SPs")])
        dt_top_marker_by_cell<-dt_top_marker_by_cell[order(-quantile_score_marker)][,head(.SD, n_top_markers),annotation_per_cell]
        name<-paste0(annotation_name,"_per_cell")
        name_score<-paste0(annotation_name,"_per_cell_score")

        colnames(dt_top_marker_by_cell)<-c(eval(name), "marker","marker_type","gene_impact_score_per_celltype_cell","weight","SPs")
    ...
    if ("score_cell" %in% group_markers_by){
      ...
      if(!is.null(condition_group_info)){
        if(is.null(celltype_group_info)){ # only condition group
          ...
          dt_top_marker_condition<-merge(dt_top_marker, condition_table, by="cell")
          dt_top <- unique(dt_top_marker_condition[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","annotation_per_cell","condition","cell_type")][,c("condition","cell_type","annotation_per_cell","marker","marker_type","quantile_score_marker","weight","SPs")])
          dt_top<-unique(dt_top[,c("annotation_per_cell","condition","cell_type","marker","marker_type","quantile_score_marker","weight","SPs")])
          dt_top_marker_by_cell<-dt_top[order(-quantile_score_marker)][,head(.SD, n_top_markers),c("annotation_per_cell","condition")]
          ...
          colnames(dt_top_marker_by_cell)<-c(eval(name), eval(condition_group_info),eval(celltype_group_info),"marker","marker_type","gene_impact_score_per_score_cell","weight","SPs")
        ...
        if(!is.null(celltype_group_info)){
          ...
          dt_top <- unique(dt_top_marker_condition[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","annotation_per_cell","cell_type")][,c("cell_type","annotation_per_cell","marker","marker_type","quantile_score_marker","weight","SPs")])
          dt_top<-unique(dt_top[,c("annotation_per_cell","cell_type","marker","marker_type","quantile_score_marker","weight","SPs")])
      ...
      if(is.null(celltype_group_info) & is.null(condition_group_info)){
        dt_top <- unique(dt_top_marker[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","annotation_per_cell")][,c("cell","annotation_per_cell","marker","marker_type","quantile_score_marker","weight","SPs")])
        dt_top_marker_by_cell<-unique(dt_top[,c("annotation_per_cell","marker","marker_type","quantile_score_marker","SPs")])
        dt_top_marker_by_cell<-dt_top_marker_by_cell[order(-quantile_score_marker)][,head(.SD, n_top_markers),annotation_per_cell]
        name<-paste0(annotation_name,"_per_cell")
        name_score<-paste0(annotation_name,"_per_cell_score")

        colnames(dt_top_marker_by_cell)<-c(eval(name), "marker","marker_type","gene_impact_score_per_score_cell","SPs")
