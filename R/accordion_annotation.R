#' Automatically annotating single-cell populations with the built-in Cell
#' Marker Accordion database
#'
#' This function performs cell types annotation exploiting the built-in
#' Accordion gene marker database. It takes in input either a Seurat object or a
#' raw or normalized count matrix and return in output the cell types assignment
...
accordion_annotation<-function(data,
  ...
  if(is.null(cell_types)){
    ...
    if(length(input_cell_type_not_in_accordion) == 1){
      if(uniqueN(accordion_marker$cell_type) == 0){
        ...
        ct_not_present<-knitr::combine_words(input_cell_type_not_in_accordion[1:length(input_cell_type_not_in_accordion)])
        ct_present<-knitr::combine_words(unique(accordion_marker$cell_type))
  ...
  if ("cell" %in% annotation_resolution ){
    ...
    if("celltype_cell" %in% group_markers_by){
      ...
      dt_top_marker_per_cell<-as.data.table(dt_top_marker_by_cell)[,c("annotation_per_cell","diff_score","marker","marker_type","EC_score","specificity","score","quantile_score_marker")]
      colnames(dt_top_marker_per_cell)<-c(eval(name),eval(name_score), "marker","marker_type","EC_score","specificity", "gene_impact_score_per_cell","gene_impact_score_per_celltype_cell")
