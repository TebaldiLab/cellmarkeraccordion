    ...
    if("celltype_cell" %in% group_markers_by){
      ...
      if(!is.null(condition_group_info)){
        if(is.null(celltype_group_info)){ # only condition group
          ...
          colnames(dt_top_marker_by_cell)<-c(eval(name), eval(condition_group_info),eval(celltype_group_info),"marker","marker_type","gene_impact_score_per_celltype_cell","EC_score","specificity")
