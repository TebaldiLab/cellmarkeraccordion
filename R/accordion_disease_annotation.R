#' Automatically annotating aberrant single-cell populations with the built-in
#' Cell Marker Accordion disease database
#'
...
#' types assignment (added to the Seurat object or as a list).
#'
...
#'   columns respectively. Default is “accordion_disease”.
...
#'   a nested list named "accordion_disease" is created. If
...
#'   stored in the misc slot of the object (object@misc@accordion_disease). If
...
#' objects, is stored in the “misc@accordion_disease” slot. If a count matrix
...
#' \itemize{
#' \item{accordion_disease}{list containing detailed information of cell types annotation.}
#' }
#'
#' @import scales
#' @import plyr
...
#' @import ontoProc
#' @export
accordion_disease_annotation<-function(data,
                            disease = NULL,
                            ...
                            allow_unknown = TRUE,
                            annotation_column = "accordion_disease",
  ...
  if(include_detailed_annotation_info == T){
    ...
    if ("cluster" %in% annotation_resolution){
      ...
      if("cluster" %in% group_markers_by){
        ...
        dt_top<- unique(dt_top_marker[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","seurat_clusters")][,c("cell_type","marker","seurat_clusters","quantile_score_marker","EC_score","specificity","marker_type","annotation_per_cluster")])
      ...
      if("celltype_cluster" %in% group_markers_by){
        # for each cluster retrieves first N markers
        dt_top<- unique(dt_top_marker[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","annotation_per_cluster")][,c("cell_type","marker","seurat_clusters","quantile_score_marker","EC_score","specificity","marker_type","annotation_per_cluster")])
      ...
      if(data_type == "seurat"){
        data@misc[["accordion_disease"]]<-cluster_res_detailed_annotation_info
      } else{
        info_list[["accordion_disease"]]<-cluster_res_detailed_annotation_info
  ...
  if ("cell" %in% annotation_resolution){
    ...
    dt_top <- unique(dt_top_marker[, quantile_score_marker := quantile(score,probs = top_marker_score_quantile_threshold, na.rm=TRUE), by=c("marker","marker_type","annotation_per_cell")][,c("cell_type","marker","marker_type","quantile_score_marker","cell","score","EC_score","specificity","annotation_per_cell", "diff_score")])
    ...
    if(data_type == "seurat"){
      if(is_empty(data@misc[["accordion_disease"]])){
        data@misc[["accordion_disease"]]<-cell_res_detailed_annotation_info
      } else {
        data@misc[["accordion_disease"]]<-append(data@misc[["accordion_disease"]], cell_res_detailed_annotation_info)
      }
    } else{
      if(is_empty(info_list[["accordion_disease"]])){
        info_list[["accordion_disease"]]<-cell_res_detailed_annotation_info
      } else{
